...
{
    flgs = (FLAGS *)malloc(sizeof(FLAGS));
    uint8_t argIndex = 1;
        ...
        if ((dp = readdir(dir)) != NULL) {
            ...
            if (dp->d_name[0] != '.') {
                ...
                if (fileName[strlen(fileName) - 1] != '/') {
                    strncpy(fileName + strlen(fileName), "/", 1);
                }
                strncpy(fileName + strlen(fileName), &dp->d_name[0], strlen(&dp->d_name[0]));

                /* Get lstat info. */
                if (lstat(fileName, &stat) == STAT_FAIL_CODE) {
                    fprintf(stderr, "ls: %s: ", fileName);
                    perror("");
                    free(flgs);
                    exit(1);
                }
                ...
                if (S_ISDIR(statBuf.st_mode)) {
                    OpenDirAndPrintContents(fileName);
                }
            }
        }
    } while (dp != NULL);
...
static void PrintSimpleNameWithIno(char *dirName, char *fileName) {
    ...
    strncpy(name, dirName, strlen(dirName));
    if (name[strlen(name) - 1] != '/') {
        strncpy(name + strlen(name), "/", 1);
    }
    strncpy(name + strlen(name), fileName, strlen(fileName));
            
    /* Get file info using stat system call. */
    if (lstat(name, &statBuf) == STAT_FAIL_CODE) {
...
static void PrintFileDescLine(char *dirName, char *fileName) {
    ...
    if (S_ISDIR(statBuf.st_mode)) {
        ...
        ssize_t len = 0;
        snprintf(linkName, strlen(" -> ") + 1, " -> ");
        if ((len = readlink(name,
                            linkName + strlen(linkName),
                            sizeof(linkName) - strlen(linkName)))
            == -1) {
            fprintf(stderr, "ls: %s: ", linkName);
            perror("");
            free(flgs);
            exit(1);
        }
